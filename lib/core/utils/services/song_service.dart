...
class SongService {
  ...
  /// Lists all songs on the device using platform-specific implementation.
  /// Returns a list of Song objects if permission is granted, otherwise an empty list.
  Future<List<Song>> listSongs() async {
    try {
      // First check if we have the necessary permissions
      bool hasPermission = await requestPermissions();
      if (!hasPermission) {
        log("Permission denied to access songs");
        return [];
      }
      ...
      // Convert the result to a list of Song objects
      return result
          .cast<Map<dynamic, dynamic>>()
          .map((map) => Song.fromMap(Map<String, dynamic>.from(map)))
          .toList();
    } on PlatformException catch (e) {
      log("Platform error when listing songs: $e");
      return [];
    } catch (e) {
      log("Unexpected error when listing songs: $e");
      return [];
    }
  }
}
